"""
IPMC Test Suite
Tests hardware address, pin shorts, and pin continuity on IPMC boards.
...
def gpio_get_direction(serial, gpio):
    ...
    hex_str = r.split("0x")[1].split("\r")[0]
...
def main():
    ...
    if (
        "Hardware Adress is invalid (expected 0x73, read 0xff)" in results[2]
        and results[1] == 1
    ):
        print("✅ Pin connectivity test passed")
    else:
        print("❌ Pin connectivity test not passed")
